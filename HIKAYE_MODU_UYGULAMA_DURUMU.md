...
| Faz 38.6–38.12 — Modern Karakter Davranışı Yükseltmesi | `partial` | Faz 38.6'nın ilk kanonik dilimi `js/StoryDecisionTrace.js` ile başladı. `DecisionContextV2`, yalnız karar aktörüne ait kaynaklı `ActorBelief` referanslarını, gerçek aday/yetki/bedel filtresini, hedefleri ve yönlü ilişki bağlamını taşır; WorldFact içeriğini okumaz. `DecisionTraceV2`, yalnız sunulmuş ve izinli aday/PASS için üretilebilir; MAJOR/WORLD hakem kararına zorunlu bağlanır ve eylem defteri şema-9 içinde save/load edilir. Başka aktöre ait özel puan/yetki/bedel gerekçeleri oyuncu projeksiyonunda gizlenir. Hedefli gizli-olgu enjeksiyonu, aday dışı ret, çapraz doğrulama, birebir kayıt/yükleme ve eski Faz 37–38 regresyonları geçti. Tetikleyici olay, rol-kanun-kurum sınırı ayrıştırması, risk/psikoloji katkısı ve gerçek UI açıklaması açık olduğundan Faz 38.6 ile üst yükseltme `partial`dır. |
