...
| Faz 38.11–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `partial` | Karakter tier yükselmesinin ilk üç dilimi çalışıyor. Gizlilik korumalı sohbet QA hattının ilk parçası da kuruldu: gerçek EXE görünür oyuncu/karakter turlarını `qa-runtime/story-dialogue-log.jsonl` içine kaynak ve LLM durumu ile yazar; sistem istemi, gizli inanç ve ham dünya verisi yazmaz, 8 MB sınırında döndürür. Müşteri-hizmetleri dili deterministik kalıplardan çıkarıldı ve LLM çıktı kapısında reddediliyor. Hedefli sohbet anlama/tanı ile 50 turluk uzun diyalog probları geçti. Doğal uzun koşu karakter dağılımı, aktivist dışı kanonik olay adaptörleri ve bilgi filtreli neden UI'sı eksiktir. |
