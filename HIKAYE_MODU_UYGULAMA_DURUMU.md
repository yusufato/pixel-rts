...
**Son kapanan faz:** Faz 38 — LLM Karakter Hakemi

**Aktif çalışma zamanı:** Faz 38.5 — Aynı oturumda deterministik çok turlu günlük sohbet
**Aktif uygulama sırası:** Faz 38.5'in karaktere özgü cevap çeşitliliğini ve güvenli model gerçekleştirmesini, merkezi DOM'u asenkron yeniden çizmeden kur
...
| Faz 38.7 — Önyargı, Geçici Stres ve Kamu Personası | `partial` | İlk kanonik dilim `js/StoryCharacterBehavior.js` ile kuruldu. Her gerçek aktörün en çok iki `CANONICAL_CORE_AXIS` kaynaklı, `2500 bp` tavanlı ve bu dilimde `scoreEffect:0` olan kararlı bias profili vardır. Kamu personası mevcut `voiceProfile`dan türetilir ve `mechanicalTruthMutable:false` taşır. Geçici stres yalnız aktörün tuttuğu ActorBelief ile aynı kaynak olaya bağlanınca açılır; aktör başına sekiz kayıt tavanı, `30–1800 sn` yarı ömür, üstel sönüm ve `10 bp` altında kapanış vardır. Sahte olay stresi reddedildi; `8000→4000` tam yarı ömür, ardından kapanış; farklı bias profilleri, sıfır refah/dünya mutasyonu, doğrulama ve birebir save/load hedefli probda geçti. Yeni `character-behavior` scheduler görevi ve kayıt alanı eklendi. Göç `knownTop` listesindeki bayat konuşma/müzakere/sözleşme/şirket/ekonomik-AI eksikleri de düzeltildi. Hızlı göç probunda ortaya çıkan çift `disconnect()` yarışı `child.connected` kapısıyla giderildi. Bias/stres henüz karar puanına kontrollü katkı vermediğinden ve kamu/özel ifade ayrımı konuşma gerçekleştiricisine bağlanmadığından faz `partial`dır. |
| Faz 38.8–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `planned` | Faz 38.7 kabulü kapandıktan sonra olay etiketli ilişki yorumu ve bağlamsal hafıza geri çağrımıyla devam eder. |
