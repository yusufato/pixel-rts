...
| Faz 38.11–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `partial` | Karakter tier yükselmesinin ilk üç dilimi çalışıyor. Gizlilik korumalı sohbet QA hattı gerçek EXE görünür turlarını `qa-runtime/story-dialogue-log.jsonl` içine yazar. İlk gerçek kayıt sonrası oyuncu konumu/askerî tehdit `UNVERIFIED_PLAYER_REPORT` sınırına, doğal askerî yardım `REQUEST_MILITARY_SUPPORT` niyetine bağlandı. Muhatap konumu bilinmiyorsa şehir uydurulmaz; oyuncu konumu yalnız “söyledin, doğrulayamıyorum” biçiminde anılır. Tekrar itirazı, rol, kamu önceliği ve konum soruları ayrı odağa sahiptir. Kanıt-temelli cevap LLM'e yeniden yazdırılmaz; genel LLM kapısı servis-botu eş anlamlısını, tam/semantik tekrarı ve doğrulanmamış bölgeyi olgu gibi aktarmayı reddeder. Gaziantep–Halep yalan dizisi zorunlu regresyondur; hedefli sohbet ve 50 turluk uzun diyalog kapıları geçer. Doğal uzun koşu karakter dağılımı, aktivist dışı kanonik olay adaptörleri ve bilgi filtreli neden UI'sı eksiktir. |
