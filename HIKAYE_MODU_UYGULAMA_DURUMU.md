...
| Faz 38.11 — Karakter Katmanları ve Kohorttan Yükselme | `partial` | `js/StoryCharacterActivation.js`; ilk salt-okunur aday katmanı gerçek Faz 23 nüfus kohortunu gerçek Faz 26 bölgesel toplumsal eylem katılımıyla kesiştirir. Yerel şiddet, seferberlik ve `PROTEST/STRIKE/UPRISING` aşaması sınırlı puandan `AGGREGATE/MINOR/RELEVANT/MAJOR/WORLD` düzeyi üretir; adayın kohortu, bölgesi, mesleği, eğitimi, hareketi, sorun türü ve suçlanan aktörü kaynaklıdır. Zorlanmış ayaklanma fikstürü gerçek çalışan kohortunu `WORLD` adayı yaptı. Bu ilk dilim özellikle isim, kimlik veya “önder” uydurmaz: `identityActorId:null / CANDIDATE_ONLY_NO_PERSON_CREATED`, nüfus ve karakter sayısı değişmez, sorgu dünya nötr ve feature-off güvenlidir. Kalıcı isim üretimi, adayın kişi olarak kohorttan ayrılmadan yükseltilmesi, activation/deactivation defteri, olay çeşitleri ve hafıza korunumu henüz eksiktir. |
