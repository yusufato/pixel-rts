...
**Aktif uygulama sırası:** Faz 38.3 — Söz, Sır, Borç ve Pazarlık Defteri
...
| Faz 38.3 — Söz, Sır, Borç ve Pazarlık Defteri | `partial` | Gerçek hazır konuşmadan idempotent/sürümlü vaka çalışıyor. Süreli sözler `OPEN→KEPT/BROKEN`; özel ActorBelief ve kaynaklı sızıntı/ihanet ilişkiler ile SECRET/BETRAYAL/PROMISE/DEBT hafızasına bir kez yazılır. Dört şart zorunludur. Mekanik ön-kontrol gerçek sevkiyat/depo/kaynak/sahiplik/sipariş-sözleşme, kanonik birim, fiziksel doluluk, ödeme ve ceza yürütücülerini denetler. `READY` vaka UI'dan etkinleştirildiğinde şirket escrow'su ayrılır, kanonik ticaret amendment'ı gerçek rotayı değiştirir ve teslim son tarihi dünya takviminde izlenir. Zamanında teslim `FULFILLED/KEPT` ile satıcı uzlaşması ve B2B lot mülkiyeti; gecikme `BREACHED/BROKEN` ile escrow iadesi, tekil ceza ve ilişki ihlali üretir. İki yol beş defterde idempotent ve save/load birebirdir. Önceden ödeme bağlı yük çift ödeme yerine `SHIPMENT_PAYMENT_ALREADY_BOUND` ile durur; alıcıdan alıcıya devir için genel `MechanicalContractV1`, insan kör ses skoru ve ağır scheduler kabulü açık olduğundan faz `partial` kalır. |
| Faz 38.4+ | `missing` | Senaryo laboratuvarı ve ilk tam sohbet çalışma alanı henüz uygulanmadı. |
