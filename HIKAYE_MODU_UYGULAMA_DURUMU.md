...
| Faz 38.11–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `partial` | Karakter tier yükselmesinin ilk üç dilimi çalışıyor. Gizlilik korumalı sohbet QA hattı gerçek EXE görünür turlarını `qa-runtime/story-dialogue-log.jsonl` içine yazar. İki gerçek oyuncu döngüsünden sonra konum/tehdit/bütçe beyanları `UNVERIFIED_PLAYER_REPORT`; doğal askerî yardım `REQUEST_MILITARY_SUPPORT` olur. Kimlik, rol ve rol çelişkisi, kuruluş bağı, ilişki bilgisi, muhatap konumu, öznel yargı, hitap, sorgulama üslubu, tekrar/fallback itirazı ile şirket/fabrika niyeti kanıt-temelli söylem odağına bağlandı. `UNKNOWN` veya kanıt-temelli cevap LLM'e verilmez; model artık şirket, şehir, ortak geçmiş veya oyuncu beyanından gerçek üretemez. Genel LLM kapısı servis-botu eş anlamlısını, tam/semantik tekrarı ve doğrulanmamış bölgeyi olgu gibi aktarmayı reddeder. İki gerçek konuşma dizisi zorunlu regresyondur; hedefli sohbet, Faz 38.5 çalışma alanı ve 50 turluk uzun diyalog kapıları geçer. Doğal uzun koşu karakter dağılımı, aktivist dışı kanonik olay adaptörleri ve bilgi filtreli neden UI'sı eksiktir. |
