...
| Faz 38.11 — Karakter Katmanları ve Kohorttan Yükselme | `partial` | `js/StoryCharacterActivation.js`; salt-okunur aday katmanı gerçek Faz 23 nüfus kohortunu gerçek Faz 26 bölgesel toplumsal eylem katılımıyla kesiştirir. Yerel şiddet, seferberlik ve `PROTEST/STRIKE/UPRISING` aşaması sınırlı puandan `AGGREGATE/MINOR/RELEVANT/MAJOR/WORLD` düzeyi üretir; adayın kohortu, bölgesi, mesleği, eğitimi, hareketi, sorun türü ve suçlanan aktörü kaynaklıdır. Aday sorgusu kişi yaratmaz. İkinci dilimde `story-character-activation-ledger-1` kalıcı yükseltme defteri açıldı: yalnız `RELEVANT/MAJOR/WORLD` aday deterministik kimlikle isimli temsilciye dönüşebilir; `AGGREGATE/MINOR` API çağrısıyla zorlanamaz. Aktör `activationOrigin`, defter ise kohort/hareket/düzey/puan/aktör çapraz bağını taşır ve aynı aday idempotenttir. İsimli kişi yeni insan değildir: `REPRESENTATIVE_INCLUDED_IN_COHORT / populationDelta:0`; kohort ve dünya nüfusu değişmez. Yükselme kalıcı hafıza izi üretir. Kaynak hareket sönünce aktör silinmez, `DORMANT_SOURCE/MINOR` düzeyine iner; pahalı karar uygunluğu kapanırken kimlik, hafıza ve ilişki korunur. Save/load iki tarafı ve dormancy durumunu korur; yeni kampanya eski promosyonları temizler, bozuk/yetim defter restore edilmez. Aktivist dışı ihbarcı, yerel lider, sembolik işçi, viral medya figürü adaptörleri; otomatik yükseltme bütçesi ve scheduler entegrasyonu henüz eksiktir. |
| Faz 38.11–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `planned` | Karakter tier yükselmesi ve gizlilik korumalı QA/UI sırasıyla gelecektir. |
