...
| Faz 38.9 — Rol Adaptörleri ve Kurumsal Müzakere | `partial` | `js/StoryCharacterRoleAdapters.js`; rol veya görünen unvan mekanik yetki sayılmaz. Mevcut kadroda 48 şirket yöneticisi kanonik şirket defterine, 16 hükümet/askerî makam sahibi gerçek `officeHolder` kaydına bağlandı; gerçek makamı bulunmayan 96 unvanlı karakter `UNAVAILABLE` kaldı. Makamın teklif/onay/icra yolları yalnız kanonik `authorityGrants` kayıtlarından türetilir. Gerçek yürütme karakterinin seferberlik teklifi `PENDING_APPROVAL`, gerçek silahlı kuvvetler komutanının onayı `AUTHORIZED`, yürütmenin icrası `EXECUTED` üretti; ikinci kurum defteri açılmadı ve Faz 29 `RECORD_ONLY` fiziksel sınırı korundu. Makam incelemesi aynı seferberlik için kaynaklı karakter eksenlerinden deterministik `APPROVE / OBJECT / REJECT` üretebilir: itiraz isteği beklemede bırakır, yalnız zorunlu gerçek makamın reddi kapalı gerekçe kodlu aktör+kurum makbuzuyla `DENIED` yapar. LLM/rastgelelik karar vermez; ret fiziksel etki üretmez ve save-load birebirdir. Kişinin ROLE/PERSONAL hedefleri ayrı gösterilir; kurum ve organizasyon hedef defteri henüz olmadığı için bunlar kurum hedefi diye kopyalanmaz. 16 ajan için kimlikteki servis bağı yalnız `CONTRACT_ONLY`dır; ayrı istihbarat yürütücüsü ve medya karakter/kurum defteri bulunmadığı katalogda açıkça yazılır. Yasama kurumu kanoniktir fakat isimli makam sahibi Faz 35 kimlik defterinde yoktur; adaptör vekil karakter uydurmaz. Genel `NEGOTIATE` yalnız karakter ilişkisi olarak kalır. Şirket kurumsal onayı, itirazın kalıcı/yeniden görüşülebilir kaydı, yasama kadrosu ve ayrıntılı alt roller eksik olduğundan faz tamamlanmadı. |
| Faz 38.10–38.12 — Modern Karakter Davranışı Yükseltmesinin Kalanı | `planned` | Türetilmiş güç/kariyer, karakter tier yükselmesi ve gizlilik korumalı QA/UI sırasıyla gelecektir. |
