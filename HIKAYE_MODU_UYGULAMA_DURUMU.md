...
**Son kapanan faz:** Faz 37 — Karakter Eylem Adayları

**Aktif uygulama sırası:** Faz 38 — LLM Karakter Hakemi
...
| Faz 38 — LLM Karakter Hakemi | `active / partial` | `js/StoryCharacterArbiter.js`; Faz 37'nin en çok sekiz doğrulanmış adayından `PROPOSE/PASS` seçen sürümlü kapalı-seçim zarfı, enum konuşma planı, katı alan/aday/hedef doğrulaması, deterministik fallback ve 32 bağlamlık önbellek kuruldu. Uydurma aday, eylem uyuşmazlığı, gizli oran enjeksiyonu ve bozuk JSON reddedildi; dünya mutasyonu ve gizli bağlam sızıntısı sıfır. Worker `0,7 sn`, açık/kapalı 60 sn karma eşit. Gerçek 5 GB model kalite/latans, canlı tüketim ve tekrar önleme açık. |
| Faz 38.1+ | `missing` | Faz 38 sözleşme ve fallback kabulü tamamlanmadan başlatılmayacak. |
...
- İlk dikey `story-character-arbiter-1`dir. Mevcut Faz 37 seçicisi ve domain doğrulayıcısı kanonik aday sahibidir; hakem bunları kopyalamaz veya genişletmez. En çok sekiz izinli aday, aktörün kimlik/hedef/ses profili, en çok altı yakın hafızası ve hedeflerle yönlü ilişki bağlamıyla sürümlü isteğe girer.
- Model çıktısı serbest dünya emri değildir. Yalnız `PROPOSE/PASS`, sunulmuş aday kimliği, eşleşen eylem/hedef ve `opening/tone/address/emphasis` enum konuşma planı kabul edilir. Model sayı, bedel, olasılık, yeni hedef, yeni eylem veya fiziksel sonuç yazamaz. Bu ilk dilimde çıktı `proposalOnly:true / worldMutation:false`tır.
- Bağlam yabancı `regionId`, servis kimliği, altyapı hasarı/kapasitesi, başarı/tespit/atıf oranı taşımaz. Uydurma aday `CANDIDATE_NOT_OFFERED`, eylem uyuşmazlığı `ACTION_MISMATCH`, `successChanceBps` alanı `UNKNOWN_FIELD`, bozuk JSON `OUTPUT_NOT_OBJECT` ile reddedildi. Hepsi aynı bağlamda birebir deterministik fallback'e döndü. Geçerli ve markdown çitli tek JSON kabul edildi.
- Yerel model adaptörü mevcut `llmEnrich` doğrulama kapısını kullanır; doğrulanmış sonuç ve fallback 32 bağlamlık salt öneri önbelleğinde tutulur. Model hazır değilse oyun beklemez. Bu dilim modeli otomatik yükleyip 5 GB RAM harcatmaz ve henüz öneriyi scheduler dünya komutuna dönüştürmez.
- Hedefli doğrudan probda dört kanonik aday, deterministik istek/fallback, sıfır gizli bağlam sızıntısı ve mutasyon öncesi/sonrası eşit `e90f4044…3bd6d` karma görüldü. Worker serileştirme yolu `0,7 sn`de geçti. `characters.llmArbiter` açık/kapalı 60 saniye A/B iki tarafta `066bde9fa6e9a76134acf6b2f834b6e98f5ab24677851664a09b631fd3f5c69b` ve geçerli Faz 37 defteri verdi.
- Açık kabul borçları: gerçek paketli 5 GB modelle kalite/latans ve kabul/ret oranı; model planının canlı karakter karar/konuşma durumunda güvenli tüketimi; kayıt/yükleme; son konuşma/söz/hitap hafızası; yakın dönem cümle ve hitap tekrar denetimi; tam 60-görev regresyonu. Bunlar bitmeden Faz 38 kapanmaz ve Faz 38.1 başlamaz.
