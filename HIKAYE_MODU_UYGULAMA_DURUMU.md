...
| Faz 38.9 — Rol Adaptörleri ve Kurumsal Müzakere | `partial` | `js/StoryCharacterRoleAdapters.js`; rol veya görünen unvan mekanik yetki sayılmaz. Mevcut kadroda 48 şirket yöneticisi kanonik şirket defterine, 16 hükümet/askerî makam sahibi gerçek `officeHolder` kaydına bağlandı; gerçek makamı bulunmayan 96 unvanlı karakter `UNAVAILABLE` kaldı. Makamın teklif/onay/icra yolları yalnız kanonik `authorityGrants` kayıtlarından türetilir. Kişinin ROLE/PERSONAL hedefleri ayrı gösterilir; kurum ve organizasyon hedef defteri henüz olmadığı için bunlar kurum hedefi diye kopyalanmaz. 16 ajan için kimlikteki servis bağı yalnız `CONTRACT_ONLY`dır; ayrı istihbarat yürütücüsü ve medya karakter/kurum defteri bulunmadığı katalogda açıkça yazılır. Genel `NEGOTIATE` yalnız karakter ilişkisi olarak kalır, şirket/devlet işlemi uydurmaz. Salt-okunur, deterministik, özellik-bayraklı ilk adaptör probe'u temizdir. Yetkili teklif→itiraz→onay→uygulama zinciri ile CEO/CFO/CTO, askerî komuta, medya ve istihbarat alt rolleri henüz eksik olduğundan faz tamamlanmadı. |
