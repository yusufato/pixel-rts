...
| EXT-ACT-018 | Kapalı-seçim LLM karakter hakemi ve deterministik fallback | Kritik | `in-progress-phase38` | `story-character-arbiter-1`, Faz 37'nin en çok sekiz kanonik adayından yalnız `PROPOSE/PASS` ve enum konuşma planı kabul eder. Yeni eylem/hedef/sayı/olasılık/dünya sonucu yasaktır. Uydurma aday, eylem uyuşmazlığı, gizli oran alanı ve bozuk JSON ayrı kodlarla fallback'e döner; bağlamda yabancı konum/servis/hasar/kapasite/olasılık sızıntısı yoktur. Worker `0,7 sn`, açık/kapalı 60 sn karma eşit. Gerçek 5 GB model kalite/latans, canlı tüketim, kayıtlı konuşma durumu ve tekrar önleme açıktır. |
