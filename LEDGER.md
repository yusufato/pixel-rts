...
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` action-scope + movement-family calibration / `aac006e`, `67b89dc`
...
- **Evidence:** Dar testler geçti. BGE sınırı `111/63/0`, kör erişim `false`. Contract-frame kolu macro-F1 `0,567021`, fark `+0,128875`, risk `0/0`, eylem recall `1`, OOD ortalama/en-kötü `0,066667/0,2`; contract+bluff kolu `0,723965`, `+0,285819`, risk `0/0`, bütün yüksek-risk recall geçer ve OOD `0,133333/0,2`. `eligibleModelIds=[]`, `createNewBlindEpoch=false`.
- **Implication for future audits:** Kapadokya örneğini yeniden cümle bazlı yasaklama ve hareket fiillerini ayrı niyet sınıfları gibi çoğaltma. Kanonik predicate kapısını koru. Kalan engel eylem recall'u veya yüksek-risk yanlış pozitif değil, bağımsız katlarda `UNKNOWN` girdinin bilgi/sohbet sınıfına sızmasıdır; sıfır OOD kanıtlanmadan yeni blind/runtime/%90+ iddiası açma.
