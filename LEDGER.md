...
## 2026-09-01 — En yakın OOD prototipi bağımsız ailelere genellenmedi
- **Type:** Refuted
- **Source:** `phase-38-turkish-semantic-intent-router` OOD-max karşı-temsili / `1174129`
- **What happened:** Oyun içi sınıflar centroidte bırakılırken `UNKNOWN` için tek ortalama yerine en yakın gerçek OOD prototipi ayrı karşı-aday yapıldı. Sentetik çok-modlu örnek ve tam kalibrasyon fit'i başarılı oldu; hipotez harcanmış blind'a erişmeden üç dış katta ölçüldü.
- **Evidence:** Sınır `99/51/0`, `blindTestAccessed=false`. OOD-max kolu macro-F1 `0,641830`, minimum kat `0,588235`, taban farkı `+0,167571`, risk `0/0` ve geçen yüksek-risk recall kapıları verdi. Tam fit OOD `3/3`; dış-kat OOD yalnız `1/3`, ortalama/en-kötü yanlış kabul `0,666667/1`. Standart centroid aynı OOD sonucu ve daha yüksek `0,664706` macro-F1 verdi; `eligibleModelIds=[]`, `createNewBlindEpoch=false`.
- **Implication for future audits:** Üç OOD prototipini centroid/max/top-K arasında yeniden tarama ve tam-fit OOD başarısını genelleme kanıtı sayma. Yeni blind açmadan önce bağımsız OOD aile kapsamını/taksonomisini tasarla; v2 blind satırlarını negatif prototipe dönüştürme.

## 2026-09-01 — On iki aileli OOD taksonomisi yanlış kabulü büyük ölçüde düşürdü
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` bağımsız OOD kapsamı / `62612c5`
- **What happened:** Meta-oyun, güncel gerçek dünya, programlama, sağlık, çeviri, yaratıcılık, kişisel tavsiye, seyahat, hukuk, rol aşımı, anlamsız dil ve akademik soru için 12 ayrı prototip ve 12 ayrı kalibrasyon ailesi tek tek gold yapıldı; blind değiştirilmedi.
- **Evidence:** Corpus `466` cümle, `426` aile, `366` gold; sınır `111/63/0`, `blindTestAccessed=false`. Seçilen frame-centroid OOD ortalama/en-kötü `0,066667/0,2` ve UNKNOWN recall `0,933333` verdi, fakat macro-F1 farkı `+0,072109`, risk `1/1`, tehdit/eylem/blöf recall `0` kaldı. Contract+bluff kolu `0,719390`, `+0,279852`, geçen recall; OOD `0,133333/0,2`, risk `1/1` verdi. `eligibleModelIds=[]`, `createNewBlindEpoch=false`.
- **Implication for future audits:** OOD kapsamını yeniden üç örneğe düşürme veya taksonomi etkisizdi deme; yanlış kabulte büyük ve ölçülmüş iyileşme vardır. Buna rağmen taksonomiyi ürün kabulü sayma. Yeni blind'dan önce etkili adayları kanonik oyun eylemi ve çözülebilir dünya varlığıyla doğrulayan kapsam kapısını bağımsız kalibrasyonda geçir.

## 2026-09-01 — Eylem kapsamı yanlış pozitifi sildi, hareket ailesi recall'u geri getirdi
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` action-scope + movement-family calibration / `aac006e`, `67b89dc`, `47dd784`
- **What happened:** Etkili eylem adayı için kanonik oyun predicate'i zorunlu yapıldı. Bu güvenlik düzeltmesinin açığa çıkardığı gerçek hareket-emri recall kaybı, tek cümle istisnası yerine getirme/götürme/taşıma/sevk/yönlendirme/gönderme ailesinin konum, emir ve istek kanıtlarıyla kapatıldı.
- **Evidence:** Dar testler geçti. BGE sınırı `111/63/0`, kör erişim `false`. Contract-frame kolu macro-F1 `0,567021`, fark `+0,128875`, risk `0/0`, eylem recall `1`, OOD ortalama/en-kötü `0,066667/0,2`; tek kaçak `stability0035 → SMALL_TALK`. Contract+bluff kolu `0,723965`, `+0,285819`, risk `0/0`, bütün yüksek-risk recall geçer ve OOD `0,133333/0,2`; kaçaklar `oodtaxc0011 → ASK_INFORMATION` ve `oodtaxc0008 → SMALL_TALK`. `eligibleModelIds=[]`, `createNewBlindEpoch=false`.
- **Implication for future audits:** Kapadokya örneğini yeniden cümle bazlı yasaklama ve hareket fiillerini ayrı niyet sınıfları gibi çoğaltma. Kanonik predicate kapısını koru. Salt `predicate != UNSPECIFIED` kuralı ekleme; geçerli Cermen Federasyonu soruları da mevcut parser'da belirsiz predicate taşır. Kalan engel bağımsız katlarda `UNKNOWN` girdinin bilgi/sohbet sınıfına sızmasıdır; çözülebilir oyun varlığı/alanı kanıtlanmadan yeni blind/runtime/%90+ iddiası açma.

## 2026-09-01 — Bounded domain sözleşmesi bütün dış-kat kalibrasyon kapılarını geçti
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` bounded-domain contract calibration / `5f5bc9c`
...
## 2026-09-01 — İç tutarlı yön-otoritesi bütün kalibrasyon kapılarını geçti
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` authoritative high-risk calibration / `15ff973`, `04c5408`
- **What happened:** Embedding sınıf adaylarını skorlamaya devam ederken deterministik `speechAct` yalnız adayla birebir eşleşip yüksek-risk çerçeve sözleşmesini geçtiğinde öncelik aldı. İlk turdaki bilgi-sorusu/düzeltme → eylem-talebi yanlış otoritesi, metne özel istisna yerine interrogative, ikincil ASK_INFORMATION ve correction/repair/answer çelişki kapılarıyla giderildi; blöfün ayrı epistemik sözleşmesi korundu.
- **Evidence:** İkinci BGE-M3 çalışması `111/63/0`, blind count `0` verdi. Dış-kat ortalama/minimum macro-F1 `0,773253/0,731523`, tam kalibrasyon `0,819468`, taban farkı `+0,335107`, yüksek-risk yanlış pozitif `0/0`, OOD yanlış kabul `0/0`; beş yüksek-risk recall değerinin tamamı `1`. `eligibleModelIds=[bge-m3-q8_0]`, `createNewBlindEpoch=true`.
- **Implication for future audits:** V3 spent durumunu değiştirme ve bu sonucu runtime/EXE/IPC ya da `%90+`/B1 kabulü sayma. Seçilmiş tek `bounded-domain-authoritative-high-risk-centroid-guard` kolundan sonra aile-sızıntısız v4 blind üret, checksum ile önceden mühürle ve yalnız bir kez ölç; kör sonuçtan sonra bu kalibrasyon hipotezini ayarlama.

## 2026-09-01 — V4 blind kohortunun 51 kaydı bireysel gold oldu
- **Type:** Executed
- **Source:** `phase-38-turkish-semantic-intent-router` v4 blind hazırlığı / `39bf443`, `1c9c6ef`, `d7d6818`, `4e09c26`, `31a2e5f`, `1afc683`, `11c8b90`
...
## 2026-09-01 — Taze yönsel-risk kalibrasyonu otorite genellemesini reddetti
- **Type:** Refuted
- **Source:** `phase-38-turkish-semantic-intent-router` directional-risk-v2 calibration / `e4e0abb`, `dde6a25`, `273d62f`, `453b8fd`
- **What happened:** V4 içeriğine veya skorlarına ayar için erişmeden, gerçek sır/blöf ile gizlilik talebi, vaat, soru, ret, söylenti, doğrulanmış rapor, sorgulama, suçlama, destek, ticaret, tehdit ve mecazı ayıran 20 aile tek tek gold yapıldı. Mevcut authoritative BGE kolu yalnız prototip+kalibrasyonda yeniden ölçüldü.
- **Evidence:** Deterministik yeni-kohort sonucu `3/20` doğru ve `4` yüksek-risk yanlış pozitif. BGE sınırı `111/83/0`, `blindTestAccessed=false`; ortalama/minimum macro-F1 `0,619753/0,527778`, fark `+0,255950`, OOD `0/0`, yüksek-risk yanlış pozitif `1/1`. Recall: tehdit `1`, eylem talebi `0,166667`, ticaret `1`, sır `0,5`, blöf `0,5`; `eligibleModelIds=[]`, `createNewBlindEpoch=false`.
- **Implication for future audits:** Eski `111/63/0` kalibrasyon başarısını otorite sözleşmesinin genellendiği kanıtı sayma. Sır/blöf için salt açık anahtar kelime veya kanıt-erteleme listesi ekleme; konuşanın iddia sahipliği, kanıt erişilebilirliği ve eylem yönünü birlikte çöz. Aynı geniş kalibrasyonda sıfır risk ve bütün recall kapıları geçmeden v5 blind/runtime/%90+ iddiası açma.

## 2026-09-01 — İddia sahipliği ve eylem yönü geniş kalibrasyonda geçti
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` compositional ownership-direction calibration / `fd59580`, `7736e6b`, `62a46ba`
...
## 2026-09-01 — Geniş kalibrasyon katı uzlaşmayı reddetti
- **Type:** Refuted
- **Source:** `phase-38-turkish-semantic-intent-router` semantic-consensus calibration
- **What happened:** Mevcut parser-öncelikli uzlaşma ile ham embedding top-1 ve deterministik çerçeve eşitliğini isteyen ayrı semantik-uzlaşma kolu, genişletilmiş bağımsız calibration üzerinde BGE-M3 ile ölçüldü. Kör V6 içeriği, tahminleri veya skorları kullanılmadı.
- **Evidence:** Her iki koşunun sınırı `111/113/0`, `blindTestAccessed=false` ve model SHA-256'sı `aa473d51…4047a173`tür. Eski kol `+0,113791` macro-F1 farkı, `1` yüksek-risk yanlış pozitif ve `REQUEST_ACTION=0,277778` recall verdi. Yeni top-1 uzlaşma kolu riski `0` ve OOD'yi `0` tuttu; fakat fark `+0,026781`, eylem/ticaret/sır/blöf recall'u `0,222222/0,444444/0,222222/0,111111` oldu. İki kol da `eligibleModelIds=[]` ve `createNewBlindEpoch=false` verdi.
- **Implication for future audits:** Parser sınıfını embedding önüne koşulsuz taşıyan eski uzlaşmayı veya ham embedding top-1 eşitliğini yeniden model kabul adayı yapma. Sonraki hipotez yalnız calibration'da doğru risk sınıfının semantik sıra/marj dağılımını ölçmeli; sıfır risk ve OOD ile kalite ve recall kapıları birlikte geçmeden blind, runtime, Electron/IPC veya B1 kabulü açılmamalıdır.

## 2026-09-01 — Risk-grubu sırası centroid temsil sınırını gösterdi
- **Type:** Refuted
- **Source:** `phase-38-turkish-semantic-intent-router` calibration semantic-rank / risk-group consensus
- **What happened:** Calibration kimlikleri için ham sınıf-centroid sırası makbuzlandı; global top-1 yerine yalnız beş yüksek-risk sınıfının birincisi ile deterministik çerçeve uzlaşmasını isteyen ayrı temsil kolu sınandı. Kör içerik veya skor kullanılmadı.
- **Evidence:** Sınır `111/113/0`, `blindTestAccessed=false`tır. `40` gerçek risk satırında global top-1/top-5 kapsaması `18/28`, risk-grubu top-1/top-5 kapsaması `24/40` oldu; sır ve blöf global top-5'te yalnız `4/9`dur. Risk-grubu kolu yanlış risk ve OOD kabulünü `0/0` tuttu, fakat macro-F1 farkı `+0,060541`; eylem/ticaret/sır/blöf recall'u `0,388889/0,444444/0,333333/0,111111` kaldı. Yeni blind uygunluğu açılmadı.
- **Implication for future audits:** Global top-K veya yalnız risk-grubu top-1 uzlaşmasını yeniden tarama. Daha çok calibration gold'un mevcut prototype-only centroidi kendiliğinden iyileştirdiğini varsayma; sonraki temsil araştırması fit calibration ailelerini dış-kat validation'dan ayırarak sızıntısız anchor öğrenmesini ölçmelidir. Blind/runtime/Electron/IPC/B1 kapıları kapalıdır.

## 2026-09-01 — Sızıntısız fit-anchor kalite ve güvenliği geçti, eylem talebi recall'u geçmedi
- **Type:** Measured
- **Source:** `phase-38-turkish-semantic-intent-router` leave-family-out learned-anchor calibration
