...
const STORY_HEX_INFRASTRUCTURE_SCHEMA_VERSION = 1;
const STORY_HEX_INFRASTRUCTURE_ADAPTER_VERSION = 'story-hex-infrastructure-segments-1';
...
function storyHexInfrastructureBuild(world, geography, settlements, corridors) {
    ...
    for (const corridor of corridors || []) {
        ...
        if (parent && parent.mode === 'LAND' && corridorSegmentIds[parent.id]) {
    ...
    return {
        ...
        topologyHash: storyHexInfrastructureHashText(segments.map(segment => [segment.id,
            segment.kind, segment.endpointCellIndices.join(':'), segment.corridorIds.join(','),
        ...
        diagnostics: {
            ...
            sharedSegmentCount: segments.filter(segment => segment.corridorIds
                .filter(id => graphById.get(id) && graphById.get(id).mode === 'LAND').length > 1).length,
...
function storyHexInfrastructureRestore(saved) {
    ...
    if (saved.schemaVersion !== STORY_HEX_INFRASTRUCTURE_SCHEMA_VERSION
        || saved.adapterVersion !== STORY_HEX_INFRASTRUCTURE_ADAPTER_VERSION
        || saved.networkHash !== registry.networkHash || saved.topologyHash !== registry.topologyHash
    ...
    return { ok: true, backfilled: false, registry };
}
