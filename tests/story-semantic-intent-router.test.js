...
assert.equal(blindV6CandidateRows.filter(row => row.adjudication).length, 27);
assert.equal(blindV6CandidateRows.filter(row => !row.adjudication).length, 24);
...
assert.equal(report.gold.total, 566);
...
assert.deepEqual(report.untouchedEvaluation.issues, [
    'UNTOUCHED_EVALUATION_ALREADY_SPENT:SPENT_AFTER_2026_09_01_V5_ONE_SHOT'
]);
...
assert.ok(evaluationSplits.blind_test.every(row =>
    row.sourceId.startsWith('representation-stability-v5:')));
...
assert.deepEqual(report.oodBySplit, {
    ...
    blind_test: { inDomain: 309, outOfDomain: 18 }
});
