...
assert.equal(blindV5CandidateRows.filter(row => row.adjudication).length, 45);
assert.equal(blindV5CandidateRows.filter(row => !row.adjudication).length, 6);
...
assert.equal(report.gold.total, 533);
...
assert.deepEqual(report.oodBySplit, {
    ...
    blind_test: { inDomain: 279, outOfDomain: 15 }
});
