...
assert.equal(report.gold.total, 241);
...
assert.deepEqual(report.untouchedEvaluation.gold, {
    total: 62,
    bySplit: { prototype: 10, calibration: 24, blind_test: 28 }
});
assert.equal(report.untouchedEvaluation.minimumPerClassPerEvaluationSplit, 3);
assert.equal(report.untouchedEvaluation.issues.length, 24);
...
assert.deepEqual(report.oodBySplit, {
    ...
    calibration: { inDomain: 63, outOfDomain: 6 },
    blind_test: { inDomain: 67, outOfDomain: 6 }
});
