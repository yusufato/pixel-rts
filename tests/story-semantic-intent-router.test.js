...
assert.equal(blindV6CandidateRows.length, 51);
assert.equal(new Set(blindV6CandidateRows.map(row => row.familyId)).size, 51);
assert.equal(new Set(blindV6CandidateRows.map(row => row.text)).size, 51);
...
assert.ok(blindV6CandidateRows.filter(row => row.adjudication).every(row =>
    ...
    'Only individually reviewed V6 candidates may become gold');
assert.deepEqual(Object.fromEntries([...new Set(blindV6CandidateRows.map(row =>
    row.proposalSpeechAct))].sort().map(label => [label,
    blindV6CandidateRows.filter(row => row.proposalSpeechAct === label).length])), {
    ASK_INFORMATION: 3,
...
assert.ok(blindV5CandidateRows.every(row => row.split === 'blind_test'
    && row.sourceType === 'MODEL_GENERATED_CANDIDATE'
    && row.labelStatus === 'CANDIDATE_UNREVIEWED'));
assert.equal(blindV5CandidateRows.filter(row => row.adjudication).length, 51);
...
'Only individually reviewed stress candidates may become gold');
assert.deepEqual(Object.fromEntries([...new Set(consensusStressReviewed.map(row =>
...
assert.equal(report.untouchedEvaluation.priorBlindStatus,
    'SPENT_AFTER_2026_09_01_V5_ONE_SHOT');
assert.deepEqual(report.untouchedEvaluation.evaluatedModelIds, ['bge-m3-q8_0']);
assert.deepEqual(report.untouchedEvaluation.issues, [
    'UNTOUCHED_EVALUATION_ALREADY_SPENT:SPENT_AFTER_2026_09_01_V6_ONE_SHOT'
]);
...
assert.equal(corpus.representationEvaluationPolicy.evaluatedAt,
    '2026-09-01T18:17:26.700Z');
assert.deepEqual(corpus.representationEvaluationPolicy.evaluatedModelIds,
    ['bge-m3-q8_0']);
assert.equal(corpus.representationEvaluationPolicy.evaluatedModelSha256,
...
assert.ok(evaluationSplits.blind_test.every(row =>
    row.sourceId.startsWith('representation-stability-v6:')));
...
const nonBlindOodRows = corpus.candidates.filter(row => row.split !== 'blind_test'
    && row.adjudication && row.adjudication.labels.outOfDomain === true);
const nonBlindOodProposals = buildBaselineProposals(nonBlindOodRows);
assert.equal(nonBlindOodRows.length, 33);
assert.deepEqual(nonBlindOodRows.filter(row =>
    nonBlindOodProposals.get(row.id).labels.speechAct !== 'UNKNOWN')
    .map(row => row.id), [],
'prototype and calibration OOD turns must abstain before embedding');
const nonBlindSmallTalkRows = corpus.candidates.filter(row => row.split !== 'blind_test'
    && row.adjudication && row.adjudication.labels.speechAct === 'SMALL_TALK');
const nonBlindSmallTalkProposals = buildBaselineProposals(nonBlindSmallTalkRows);
assert.equal(nonBlindSmallTalkRows.length, 22);
assert.deepEqual(nonBlindSmallTalkRows.filter(row =>
...
const nonBlindOpinionRows = corpus.candidates.filter(row => row.split !== 'blind_test'
    && row.adjudication && row.adjudication.labels.speechAct === 'ASK_PERSONAL_OPINION');
const deferredOpinionContractId = 'semantic-intent:stability0004';
const resolvedNonBlindOpinionRows = nonBlindOpinionRows.filter(row =>
    row.id !== deferredOpinionContractId);
const resolvedNonBlindOpinionProposals = buildBaselineProposals(resolvedNonBlindOpinionRows);
assert.equal(nonBlindOpinionRows.length, 7);
assert.equal(resolvedNonBlindOpinionRows.length, 6);
assert.deepEqual(resolvedNonBlindOpinionRows.filter(row =>
    resolvedNonBlindOpinionProposals.get(row.id).labels.speechAct !== 'ASK_PERSONAL_OPINION')
    .map(row => row.id), [],
'non-blind listener evaluation questions must remain personal-opinion questions');
const nonBlindGreetingRows = corpus.candidates.filter(row => row.split !== 'blind_test'
    && row.adjudication && row.adjudication.labels.speechAct === 'GREETING');
const nonBlindGreetingProposals = buildBaselineProposals(nonBlindGreetingRows);
assert.equal(nonBlindGreetingRows.length, 10);
assert.deepEqual(nonBlindGreetingRows.filter(row =>
    nonBlindGreetingProposals.get(row.id).labels.speechAct !== 'GREETING')
    .map(row => row.id), [],
'prototype and calibration greeting performances must remain greetings');
const nonBlindRelationshipRows = corpus.candidates.filter(row => row.split !== 'blind_test'
    && row.adjudication && row.adjudication.labels.speechAct === 'ASK_RELATIONSHIP');
const nonBlindRelationshipProposals = buildBaselineProposals(nonBlindRelationshipRows);
assert.equal(nonBlindRelationshipRows.length, 2);
assert.deepEqual(nonBlindRelationshipRows.filter(row =>
