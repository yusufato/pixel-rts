...
**Aktif uygulama sırası:** Toplantı ve görev sonuçlarının yönlü ilişki fişleri
...
Şirket bütçeli görevler ayrı kapsamdır; yönlü ilişki fişleri kapanmadan Faz 39
aktif değildir.
...
| Faz 38.13 — Oynanabilir Karakter Etkileşimi ve Resmî Toplantılar | `partial` | İlk on beş dikey çalışıyor. `ConversationCaseV1` altı kip; `TaskOfferV2` ödülsüz kişisel istek ile yetkili/bedelli devlet görevini ayrı union dallarında korur. Kurumsal dal yalnız gerçek Silahlı Kuvvetler makam sahibi aynı zamanda gerçek komutan payer olduğunda Faz 29 `COMMISSION_PAID_CONTACT_TASK` isteğini yürütür. Kabul 25 devlet kredisini ayrı bütçe escrow'una ayırır; hedef görüşme tek settlement ve `InstitutionalTaskReceiptV1` üretir; ret parasız, süre aşımı tek iadeli, restore yan etkisizdir. UI kurum/yasal dayanak/bedel/ödeme/makbuz durumunu hesap kimliği sızdırmadan gösterir. `MeetingCaseV1`, `MeetingMotionV1`, `InstitutionProposalIntentV1`, `MeetingClosureRecordV1` ve şema-6 `PLAYER_NOTE → CHARACTER_REPLY` zincirleri önceki görünürlük/yetki değişmezlerini korur. Hedefli test 37 kamusal tur, 5 özel kayıt, 2 karakter yanıtı yanında kabul/ret, tek ödeme/iade, çift çağrı, yetersiz bakiye, tahrif ve bütün görev durumlarında birebir save-load'u geçti. Devlet kurumu görevi kapanmıştır; şirket bütçeli görevler ayrı kapsam, yönlü ilişki fişleri açık borçtur. |
