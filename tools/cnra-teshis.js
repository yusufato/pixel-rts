...
const ESCORT = process.argv.includes('--escort');
const NOLOJ = process.argv.includes('--nolojistik');   // IZOLE kontrol: lojistik kurali KAPALI   // elenen supplyEscort'u BU vakada yeniden sina
...
const kod = [
    ...
    'BATTLE_INTEL4PRO_DELTAS.logisticsRequirement = ' + (!NOLOJ) + ';',
    'const cikti = [];',
    ...
    '    muhimmatliGorunmez:0, atis:0, ikmal:0, olen:0, ilkKuruSn:null, hasar:0, olduren:0, halede:0, kuruHalede:0 };',
    ...
    '          if (Math.hypot(f.x-u.x, f.y-u.y) <= (a.radius||3) * ((typeof TILE_PX!=="undefined")?TILE_PX:100)) { halede = true; break; } }',
    '        if (halede) { k.halede++; if (u.ammo <= 0) k.kuruHalede++; } }',
...
const TKH = r.reduce((a,x)=>a+x.kuruHalede,0);
console.log('  ikmal HALESI icinde (tum canli tik): %' + Math.round(T.halede / Math.max(1, T.canli) * 100));
console.log('  KURUYKEN halede                    : %' + Math.round(TKH / Math.max(1, T.kuru) * 100) + '   <- ikmalin ONEMLI oldugu an');
